...
def fetch_all_oms_customers() -> list:
    """Fetch all customers from OMS search API with pagination. Returns list of raw customer dicts."""
    ...
    while True:
        ...
        if total_count is None and "total_count" in data:
            ...
            logger.debug("%s [CUSTOMER_FETCH] total_count=%s", LOG_PREFIX, total_count)
        logger.info("%s [CUSTOMER_FETCH] Fetching page %s, got %s items", LOG_PREFIX, page, len(items))
